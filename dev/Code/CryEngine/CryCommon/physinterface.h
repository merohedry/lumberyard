...
struct IPhysicalWorld
{
    ...
    struct SPWIParams
    {
            ...
            intersection_params* _pip = 0, PhysicsForeignData _pForeignData = 0, int _iForeignData = 0, IPhysicalEntity** _pSkipEnts = 0, int _nSkipEnts = 0)
        {
            ...
            geomFlagsAll = _geomFlagsAll;
            geomFlagsAny = _geomFlagsAny;
            collclass = _collclass;
            pip = _pip;
            pForeignData = _pForeignData;
            iForeignData = _iForeignData;
    ...
    virtual IPhysicalEntity* CreatePhysicalEntity(pe_type type, pe_params* params = 0, PhysicsForeignData pForeignData = 0, int iforeigndata = 0, int id = -1, IGeneralMemoryHeap* pHeap = NULL) = 0;
    // CreatePhysicalEntity - this version is used when creating an a temporary entity (lifeTime) on demand for a placeholder
    virtual IPhysicalEntity* CreatePhysicalEntity(pe_type type, float lifeTime, pe_params* params = 0, PhysicsForeignData pForeignData = 0, int iForeignData = 0, int id = -1, IPhysicalEntity* pHostPlaceholder = 0, IGeneralMemoryHeap* pHeap = NULL) = 0;
    ...
    virtual IPhysicalEntity* CreatePhysicalPlaceholder(pe_type type, pe_params* params = 0, PhysicsForeignData pForeignData = 0, int iForeignData = 0, int id = -1) = 0;
    ...
    virtual void SimulateExplosion(pe_explosion* pexpl, IPhysicalEntity** pSkipEnts = 0, int nSkipEnts = 0, int iTypes = ent_rigid | ent_sleeping_rigid | ent_living | ent_independent, int iCaller = GetMaxPhysThreads()) = 0;
    ...
    virtual void DrawPhysicsHelperInformation(IPhysRenderer* pRenderer, int iCaller = GetMaxPhysThreads()) = 0;
    ...
    inline int RayWorldIntersection(const Vec3& org, const Vec3& dir, int objtypes, unsigned int flags, ray_hit* hits, int nMaxHits,
        ...
        const char* pNameTag = RWI_NAME_TAG, ray_hit_cached* phitLast = 0, int iCaller = GetMaxPhysThreads())
    {
        SRWIParams rp;
        rp.org = org;
        rp.dir = dir;
    ...
    int RayWorldIntersection(const Vec3& org, const Vec3& dir, int objtypes, unsigned int flags, ray_hit* hits, int nMaxHits,
        IPhysicalEntity* pSkipEnt, IPhysicalEntity* pSkipEntAux = 0, PhysicsForeignData pForeignData = 0, int iForeignData = 0)
    {
        IPhysicalEntity* pSkipEnts[2];
        int nSkipEnts = 0;
        if (pSkipEnt)
        {
            pSkipEnts[nSkipEnts++] = pSkipEnt;
        }
        if (pSkipEntAux)
        {
            pSkipEnts[nSkipEnts++] = pSkipEntAux;
        }
    ...
    float PrimitiveWorldIntersection(int itype, const primitives::primitive* pprim, const Vec3& sweepDir = Vec3(ZERO), int entTypes = ent_all,
        ...
        PhysicsForeignData pForeignData = PhysicsForeignData(), int iForeignData = 0, IPhysicalEntity** pSkipEnts = 0, int nSkipEnts = 0, WriteLockCond* pLockContacts = 0, const char* pNameTag = PWI_NAME_TAG)
    {
        SPWIParams pp;
        pp.itype = itype;
        pp.pprim = pprim;
